...
import { costCenterService, CostCenter, accountService, Account, documentTypeService, DocumentType } from '@/services/muhasebe.service';
import '../styles/EInvoicesPage.compact.css';
      ...
      for (let i = 0; i < files.length; i += BATCH_SIZE) {
        ...
        const results = await Promise.allSettled(
          batch.map(file => {
            ...
            return einvoiceAPI.uploadPDF(formData)
      ...
      <Card style={{ marginBottom: 16 }}>
          ...
          <RangePicker
            ...
            onChange={(dates) => {
              if (dates && dates[0] && dates[1]) {
                ...
                setFilters(rest as EInvoiceFilters);
              }
            }}
          />
          <Select
        ...
        <div style={{ marginTop: 16, borderTop: '1px solid #f0f0f0', paddingTop: 16 }}>
          <Tabs
            ...
            items={(() => {
              // Dinamik yıl listesi: 2024'ten şimdiki yıla kadar
              const currentYear = dayjs().year();
              const startYear = 2024;
              const endYear = currentYear; // Sadece şimdiki yıla kadar
              const years = [];
              
              const monthNames = ['OCAK', 'ŞUBAT', 'MART', 'NİSAN', 'MAYIS', 'HAZİRAN', 
                                  'TEMMUZ', 'AĞUSTOS', 'EYLÜL', 'EKİM', 'KASIM', 'ARALIK'];
              
              // Yılları tersine ekle (en yeni en üstte)
                ...
                years.push({
                  ...
                  children: (
                    <Tabs
                      size="small"
                      type="line"
                      defaultActiveKey={dayjs().year() === year ? dayjs().format('YYYY-MM') : undefined}
                      onChange={(monthKey) => {
                        if (monthKey === `all-${year}`) {
              ...
              <>
                ...
                >
                  <Descriptions column={3} size="small" style={{ marginBottom: 12 }} bordered>
                    ...
                    <Descriptions.Item label="Açıklama">{importPreviewData.transaction?.description || '-'}</Descriptions.Item>
                  </Descriptions>
          ...
          <>
            ...
            <Card size="small" style={{ marginBottom: 16, backgroundColor: '#fafafa' }}>
              ...
              <div style={{ marginTop: 12 }}>
                <div style={{ fontSize: 12, marginBottom: 4, color: '#666' }}>
                  Ödeme Durumu: {Number(selectedInvoiceForPayment.payment_percentage || 0).toFixed(1)}%
                </div>
                <Progress
            ...
            >
              ...
              >
                <InputNumber
                  style={{ width: '100%' }}
                  placeholder={`Kalan: ${Number(selectedInvoiceForPayment.remaining_amount || selectedInvoiceForPayment.payable_amount || 0).toFixed(2)} ₺`}
                  precision={2}
                  min={0}
                  max={Number(selectedInvoiceForPayment.remaining_amount || selectedInvoiceForPayment.payable_amount || 0)}
                  suffix="₺"
                />
              </Form.Item>
