...
print("=" * 80)
print("SON GÜNCEL CSV ANALİZİ (HALİ1)")
print("=" * 80)

# CSV'yi oku
df = pd.read_csv(csv_file, sep=';', encoding='utf-8-sig', dtype={'account_code': str, 'account_id': str})
print(f"\n📄 Toplam Satır: {len(df):,}")
print(f"📊 Kolonlar: {list(df.columns)}")

# account_id veya account_code kullan
...
if abs(total_diff) < 50 and len(csv_only) == 0:
    ...
    print("✅ Database'e yüklenebilir!")
elif abs(total_diff) < 50:
    print(f"⚠️ Bakiye OK ({abs(total_diff):.2f} TL) ama {len(csv_only)} hesap eksik - önce hesapları ekle")
else:
