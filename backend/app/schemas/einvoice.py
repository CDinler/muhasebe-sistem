...
class ImportInvoiceRequest(BaseModel):
    """E-Fatura import istek şeması"""
    invoice_type: str  # Satış, İade, Tevkifat, İstisna, Özel Matrah, İhraç Kayıtlı, SGK, Tevkifat İade, Konaklama Vergisi
    transaction_number: Optional[str] = None
    lines: List[Dict[str, Any]]  # Düzenlenmiş muhasebe satırları
    invoice_lines_mapping: Optional[List[Dict[str, Any]]] = None  # Fatura satırlarının kategori ve hesap eşleşmeleri
    
    class Config:
        from_attributes = True
    outgoing_amount: Decimal = Decimal("0")
    outgoing_archive_count: int = 0
    outgoing_archive_amount: Decimal = Decimal("0")
